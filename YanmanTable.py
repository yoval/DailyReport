...
del pd_ol_yanman
fileList = glob.glob(os.path.join(floder,'在线延迟满意明细表*.xlsx'))
filePath = [i for i in fileList if date in i][0]
print('即将合并至日表的的文件为：%s'%filePath)
...
New_pd['姓名'] = NameList
New_pd['小组'] = ['' for i in range(New_pd.shape[0])]
New_pd['团队'] = ['' for i in range(New_pd.shape[0])]
New_pd['职场'] = ['' for i in range(New_pd.shape[0])]
New_pd['上线日期'] = ['' for i in range(New_pd.shape[0])]
New_pd['员工属性'] = ['' for i in range(New_pd.shape[0])]
New_pd['BPO'] = BPOList
New_pd['完结时段'] = ['' for i in range(New_pd.shape[0])]
New_pd['业务订单号'] = [str(i) for i in New_pd['业务订单号']]
###########
#匹配排班表
XiaozhuList = []
TuanduiList = []
...
New_pd['是否1.5线'] = ydwxList
New_pd['小组'] = XiaozhuList
New_pd['团队'] = TuanduiList
New_pd['职场'] =  ZhichangList
New_pd['上线日期'] =  ShangxianriqiList
#New_pd['完结时间'] = New_pd.to_datetime(New_pd['完结时间'],format='%Y/%m/%d')
New_pd['员工属性'] = YuangongshuxingList
del df_huamingce
